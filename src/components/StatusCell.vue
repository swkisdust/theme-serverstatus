<template>
    <div :class="`inline-block w-[18px] h-[18px] rounded-[3px] shadow-cell ${bgClass}`" ref="status-cell"
        @mouseenter="isShow = !isShow" @mouseleave="isShow = !isShow">
    </div>
    <Transition name="fade">
        <div ref="tooltip" v-if="isShow" class="bg-black py-[3px] px-2 text-xs rounded-[4px] text-white"
            :style="floatingStyles">
            {{ statusText || "No Data" }}
            <div ref="floatingArrow" :style="{
                position: 'absolute',
                left:
                    middlewareData.arrow?.x != null
                        ...
                        : '',
                bottom: '-0.2rem',
            }" class="h-[6px] w-[6px] rotate-45 bg-black"></div>
        </div>
    </Transition>
</template>
