    ...
    <table class="w-full caption-bottom leading-[20px] text-[14px]">
      ...
      <tbody class="text-[14px]">
        <template v-for="row in table.getRowModel().rows" :key="row.id">
          <tr :class="`border-t ${row.index % 2 === 0 ? `bg-[#ffffff]` : `bg-[#f9f9f9]`} hover:bg-[#e6e6e6]`">
...
const columns: ColumnDef<ModelServiceResponseItem>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      const state = nezhaUtils.serviceState(nezhaUtils.formatUsage(s.total_up, s.total_down));
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h("div", { class: "flex gap-1 justify-center items-center" }, [
        states.map(state => h(StatusCell, { statusText: state.statusText, avail: state.per })),
      ]);
    },
  },
  {
    ...
    accessorFn: (row) => {
      const { sum, count } = row.delay.reduce((acc, value) => {
        if (value !== 0) {
          acc.sum += value;
          acc.count += 1;
        }
